...
AOELootCreatureCheck.IsValidLootCreature = (player, target, creature) ->
    return false unless creature\GetGUID! != target\GetGUID!
...
AOELootCreatureCheck.GetCreaturesInRange = (player, target) ->
    creatures = player\GetCreaturesInRange LootDistance, 0, 0, 2
...
AOELoot.OnLootOpen = (event, packet, player) ->
    ...
    return false unless aoe_loot_active

    target = player\GetSelection!
    return false unless AOELootCreatureCheck.IsValidLootTarget target
